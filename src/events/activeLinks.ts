...
export async function getActiveLinks(contractAddress: string, provider: ethers.providers.Provider) {
    // Read the LinkAdded and LinkRemoved events
    const linkAddedEvents = await readLinkAddedEvents(contractAddress, provider);
    const linkRemovedEvents = await readLinkRemovedEvents(contractAddress, provider);
